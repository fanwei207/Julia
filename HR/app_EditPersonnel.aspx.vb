...
#End Region
        ...
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load, Me.Load
            ...
            If Not IsPostBack Then
                ...
                If Request("id") = "" Then
                    ...
                    enterdate.Text = Format(Now, "yyyy-MM-dd")
                    ...
                    appid = Request("appid")
                    Query = " select name, appProc, appDate, arrDate, birthDay, IC, sex, place, nation, phone, address, languageLevelID, language, languageLevel,computerLevelID, computer, computerLevel, appType,marriage,marriageID" _
                                  & " from userRegistList " _
                                  & " where id=" & appid
                    reader1 = SqlHelper.ExecuteReader(chk.dsnx, CommandType.Text, Query)

                    While (reader1.Read())
                        UserName.Text = reader1(0)
                        birthday.Text = Format(reader1(4), "yyyy-MM-dd")
                        icno.Text = reader1(5)
                        nation = reader1(8)
                        dropNation.SelectedItem.Text = reader1(8).ToString()
                        Mobile.Text = reader1(9)
                        currentAddress.Text = reader1(10)

                        Dim i As Integer
                        For i = 1 To marriage.Items.Count - 1
                            If reader1(19).ToString() = "" Then
                                marriage.SelectedIndex = 0
                                Exit For
                            Else
                                If marriage.Items(i).Value = reader1(19) Then
                                    marriage.SelectedIndex = i
                                    Exit For
                                End If
                            End If
                        Next
                        'Query = "select systemCodeID from systemCode where systemCodeName =" & nation
                        'reader3 = SqlHelper.ExecuteReader(chk.dsnx, CommandType.Text, Query)
                        'While (reader1.Read())
                        '    Dim i As Integer
                        '    For i = 1 To province.Items.Count - 1
                        '        If reader3(2).ToString() = "" Then
                        '            province.SelectedIndex = 0
                        '            Exit For
                        '        Else
                        '            If province.Items(i).Value = reader3(0) Then
                    ...
                    Query = "select sexID,educationID,placeID,departmentID FROM  UsersInformation where id =" & tableid
                    reader2 = SqlHelper.ExecuteReader(chk.dsnx, CommandType.Text, Query)
                    While (reader2.Read())
                        Dim i As Integer
                        For i = 1 To province.Items.Count - 1
                            If reader2(2).ToString() = "" Then
                                province.SelectedIndex = 0
                                Exit For
                            Else
                                If province.Items(i).Value = reader2(2) Then
                                    province.SelectedIndex = i
                                    Exit For
                                End If
                            End If
                        ...
                        For i = 1 To sex.Items.Count - 1
                            If reader2(0).ToString() = "" Then
                                ...
                                Exit For
                            Else
                                If sex.Items(i).Value = reader2(0) Then
                                    sex.SelectedIndex = i
                                    Exit For
                                End If
                            End If
                        ...
                        For i = 1 To education.Items.Count - 1
                            If reader2(1).ToString() = "" Then
                                education.SelectedIndex = 0
                                Exit For
                            Else
                                If education.Items(i).Value = reader2(1) Then
                        ...
                        For i = 1 To Department.Items.Count - 1
                            If reader2(3).ToString() = "" Then
                                Department.SelectedIndex = 0
                                Exit For
                            Else
                                If Department.Items(i).Value = reader2(3) Then
        ...
        Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
            ...
            'add by mabo 2014-12-05 A类员工不能从这菜单录入
            If userType.SelectedValue = 394 Then

                ltlAlert.Text = "alert('请选择非A类的其他类型。');"
                Exit Sub
            End If
            ...
            End If
            'Added by mabo 新员工创建成功，改变其状态
            Dim tableid As Integer
            Dim dsNewUser1 As DataSet
            tableid = Request("tableid")
            strSql = "update UsersInformation set status = N'已建档',statusID = 8  where id =" & tableid
            dsNewUser1 = SqlHelper.ExecuteDataset(chk.dsnx, CommandType.Text, strSql)
            dsNewUser1.Dispose()
            '向此岗位所涉及到的所有人发送邮件
            Dim company As String
            Dim departmnet As String
            Dim process As String
            company = Request("company")
            departmnet = Request("department")
            process = Request("process")
            Response.Redirect("app_SendEmail.aspx?company=" + Request("company") + "&department=" + Request("department") + "&process=" + Request("process") + "&username=" + UserName.Text)
        ...
        Sub InitDropDownList()
            ...
            Query = "SELECT systemCodeID,systemCodeName From tcpc0.dbo.systemCode s Inner Join tcpc0.dbo.systemCodeType st On st.systemCodeTypeID=s.systemCodeTypeID Where st.systemCodeTypeName='Access Type' order by s.systemCodeID desc"
        ...
        Private Function CheckIdCard(ByVal idCard As String) As Boolean
        ...
        Protected Sub icno_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles icno.TextChanged
            If icno.Text.Length > 0 And Not CheckIdCard(icno.Text.Trim()) Then
                ltlAlert.Text = "alert('身份证号格式不正确!');"
            End If
        End Sub
