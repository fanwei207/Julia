...
Namespace tcpc
    ...
    Partial Class Performance_perf_punishlist
        Inherits BasePage
...
#End Region
        ...
        Private Function createSQL() As String
            ...
            createSQL &= " from perf_mstr m Left Outer Join (select perf_mstr_id,sum(isnull(perf_mark,0)) as mark ,perf_userid from perf_details where perf_deleted is null and perf_hist_id is null AND perf_duty = 1 group by perf_mstr_id,perf_userid ) d on d.perf_mstr_id = m.perf_mstr_id  "
            createSQL &= " LEFT JOIN tcpc0..users us ON us.userID = d.perf_userid LEFT JOIN dbo.dept_Director dep ON dep.departmentID = us.departmentID "
            createSQL &= " where m.perf_deletedby is null and m.perf_hist_id is null "
            createSQL &= " AND  (dep.DirectorId = '" & Session("uID") & "' OR dep.ViceDirectorId LIKE '%" & Session("uID") & "%' AND dep.ViceDirectorName LIKE N'%" & Session("uName") & "%' ) "
