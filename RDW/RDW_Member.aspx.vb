...
#End Region
        ...
        Sub CreateSql()
            StrSql = "SELECT m.RDW_mstrid,d.RDW_detid,a.RDW_PartnerID,m.RDW_project,m.RDW_prodcode "
            StrSql &= " ,d.RDW_TaskID,d.RDW_StepName ,d.RDW_StartDate,d.RDW_EndDate"
            StrSql &= " ,u.UserName,pu.UserName as Member,m.RDW_proddesc,m.RDW_ProdSKU  "
            StrSql &= " FROM RDW_Det d Inner Join RDW_Mstr m "
            StrSql &= " on d.RDW_mstrid=m.RDW_mstrid Inner Join RDW_Det_Ptr a "
            StrSql &= " on a.RDW_detid=d.RDW_detid  Inner Join tcpc0.dbo.Users u On u.userID = m.RDW_CreatedBy "
            StrSql &= " Inner Join tcpc0.dbo.Users pu On pu.userID = a.RDW_PartnerID "
            StrSql &= " where isnull(m.RDW_Status,'')='PROCESS'"
            StrSql &= " and isnull(a.RDW_Result,0)=0 "
            'If Session("uRole") <> 1 Then
            If ddlMode.SelectedValue = "0" Then
                StrSql &= " and a.RDW_PartnerID='" & Session("uID") & "'"
            End If
            If Session("plantCode") = "99" Then
                StrSql &= " and m.RDW_Category <> 147 "
            End If
            'End If
        ...
        Sub BindData()
            ...
            dtl.Columns.Add(New DataColumn("g_member", System.Type.GetType("System.String")))
            Dim drow As DataRow
            With ds.Tables(0)
                If .Rows.Count > 0 Then
                    ...
                    For i = 0 To .Rows.Count - 1
                        ...
                        drow.Item("g_uid") = .Rows(i).Item("RDW_PartnerID").ToString().Trim()
                        ...
                        drow.Item("g_member") = .Rows(i).Item("Member").ToString().Trim()
                        dtl.Rows.Add(drow)
                    Next
                End If
            End With
        ...
        Private Sub datagrid1_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles Datagrid1.ItemCommand
            If e.CommandName.CompareTo("g_appr") = 0 Then
                Response.Redirect("/RDW/RDW_DetailEdit.aspx?mid=" & e.Item.Cells(0).Text & "&did=" & e.Item.Cells(1).Text & "&fr=znqmb&rm=" & Now.ToString)
            End If

            If e.CommandName.CompareTo("gobom") = 0 Then
                ltlAlert.Text = "var w=window.open('/RDW/RDW_doclist.aspx?mid=" & e.Item.Cells(0).Text & "&rm=" + DateTime.Now.ToString() + "','','menubar=yes,scrollbars = yes,resizable=yes,width=800,height=500,top=0,left=0'); w.focus();"
            End If
        End Sub
        ...
        Protected Sub btnExport_Click(sender As Object, e As System.EventArgs) Handles btnExport.Click
            CreateSql()
            Dim title As String = "150^<b>Project</b>~^120^<b>Project Code</b>~^80^<b>Step No,</b>~^160^<b>Step Name</b>~^<b>Step StartDate</b>~^<b>Step EndDate</b>~^<b>Creator</b>~^<b>Member</b>~^200^<b>Project Desc</b>~^200^<b>SKU#</b>~^"
