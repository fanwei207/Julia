'!*******************************************************************************!
'* @@ NAME				:	GuSalary.aspx.vb
'* @@ AUTHOR			:	Xin Bao
'*
'* @@ DESCRIPTION		:	Code behind file for GuSalary.aspx
'*					
'* @@ REQUIRED OBJECTS	:	NA
'*
'* @@ INCLUDE FILES		:	adamFuncs.dll and Microsoft.ApplicationBlocks.Data.dll  in '/bin' : Security Functions
'*
'* @@ TEMPLATE DATE		:	August 4 2008
'*
'!-------------------------------------------------------------------------------! 
'* @@ HISTORY			:	NA
'*	
...
#End Region
    ...
    Sub SaleBind(ByVal temp As Integer)
        Dim ds As DataSet
        strSql = " Select o.id,u.userID,u.userNo,u.username,isnull(d.name,'') as dname,o.sdate,isnull(s.roleName,'') as role  From EmployeeGU o "
        strSql &= " INNER JOIN  tcpc0.dbo.users u ON u.userID=o.usercode "
        strSql &= " left outer join roles s on s.roleID=u.roleID"
        ...
        dt.Columns.Add(New DataColumn("role", System.Type.GetType("System.String")))
        ...
        With ds.Tables(0)
            If (.Rows.Count > 0) Then
                ...
                For i = 0 To .Rows.Count - 1
                    ...
                    dr1.Item("enterdate") = Format(.Rows(i).Item("sdate"), "yyyy-MM-dd")
                    dr1.Item("enterdate1") = .Rows(i).Item("sdate")
  
                    dr1.Item("role") = .Rows(i).Item("role")
    ...
    End Sub
        ...
        Sub BtnSave_click(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            If sdate.Text.Trim.Length = 0 Then
                ltlAlert.Text = "alert('日期 不能为空!');"
                Exit Sub
            End If
            ...
            If IsDate(sdate.Text.Trim) = False Then
                ltlAlert.Text = "alert('输入日期有误!');Form1.sdate.focus();"
                Exit Sub
            End If


            Dim eid As String = ""
            strSql = " select id from EmployeeGu where month(currentdate)='" & Dropdownlist2.SelectedValue & "' and year(currentdate)='" & yeartextbox.Text.Trim & "' and usercode='" & userID.Text.Trim() & "' "
            ...
            Else
                ...
                strSql = " insert into EmployeeGu (usercode,currentdate,sdate,creatby,creatday) values ('" & userID.Text.Trim() & "','" & edate & "','" & sdate.Text.Trim & "','" & Session("uID") & "',getdate())"
                SqlHelper.ExecuteNonQuery(chk.dsnx, CommandType.Text, strSql)
                workerNo.Text = ""
                workerName.Text = ""
                userID.Text = ""
