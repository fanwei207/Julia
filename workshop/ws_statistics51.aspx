...
<%@ Page Language="vb" AutoEventWireup="false" Inherits="tcpc.ws_statistics51" CodeFile="ws_statistics51.aspx.vb"
...
//with (_DiagramTarget.document)
{
	...
	D.SetBorder(2009, 2018, 0, high);
	D.SetText("年","合格率", "一次合格率");
	//D.XScale="%";
	//D.YScale="%";
//	D.Draw("#FFEECC", "#663300", false);
    ...
    for (j=0;j<9;j++)
    {
      //tmpmark0=tmpmark1;
      tmpmark1=num[j];
      //new Line(D.ScreenX(j+2002), D.ScreenY(tmpmark0), D.ScreenX(j+2003), D.ScreenY(tmpmark1), "0000ff", 2);
      new Box(D.ScreenX(j+2009), D.ScreenY(tmpmark1), D.ScreenX(j+2010), D.ScreenY(0), "ff0000",'','','','',num[j]);
    }
	j=D.ScreenX(2009)+670;
	new Bar(j, y0-10, j+90, y0+10, '', '年', '#663300');
	j=D.ScreenX(2009)-20;
	y0=y0-210;
	new Bar(j, y0-10, j+50, y0+10, '', '合格率', '#663300');
...
//	D.Draw("#FFEECC", "#663300", false);
    ...
    for (j=0;j<9;j++)
    {
      ...
      tmpmark1=num[j];
      //new Line(D.ScreenX(j+2002), D.ScreenY(tmpmark0), D.ScreenX(j+2003), D.ScreenY(tmpmark1), "0000ff", 2);
      new Box(D.ScreenX(j+2009), D.ScreenY(tmpmark1), D.ScreenX(j+2010), D.ScreenY(0), "0000ff",'','','','',num[j]);
    }
	j=D.ScreenX(2009)+670;
	new Bar(j, y0-10, j+90, y0+10, '', '年', '#663300');
	j=D.ScreenX(2009)-20;
	y0=y0-310;
	new Bar(j, y0-10, j+50, y0+10, '', '流量', '#663300');
