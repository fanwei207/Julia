...
<%@ Page Language="vb" AutoEventWireup="false" Inherits="tcpc.ws_statistics" CodeFile="ws_statistics.aspx.vb"
...
//with (_DiagramTarget.document)
{
	...
	D.SetBorder(-47,25, 0, high);
	D.SetText("<%=ddl_type.SelectedItem.text%>","合格率", "一次合格率(72小时)");
	//D.XScale="%";
	//D.YScale="%";
    ...
    for (j=1;j<73;j++)
    {
      tmpmark0=tmpmark1;
      tmpmark1=num[j];
      //new Line(D.ScreenX(j-144), D.ScreenY(tmpmark0), D.ScreenX(j-143), D.ScreenY(tmpmark1), "ff0000", 2,num[j]);
      new Box(D.ScreenX(j-47), D.ScreenY(tmpmark1), D.ScreenX(j-46), D.ScreenY(0), "ff0000",'','','','',num[j]);
    }
	j=D.ScreenX(-47)+610;7
	new Bar(j, y0-10, j+220, y0+10, '', '小时', '#663300');
...
//with (_DiagramTarget.document)
{
    ...
    var high=0;
       ...
       sql="SELECT dispx, qty from SZXWS.LineData_SZX.dbo.ls_display where userid='" & Session("uID") & "' order by id"
       ds = SqlHelper.ExecuteDataset(chk.dsnx, CommandType.Text, sql)
       With ds.Tables(0)
          If .Rows.Count > 0 Then
             For i = 0 To .Rows.Count - 1
	...
	D.SetBorder(-47,25, 0, high);
	D.SetText("<%=ddl_type.SelectedItem.text%>","流量", "流水线流量(72小时)");
	//D.XScale="%";
	//D.YScale="%";
    //	D.Draw("#FFEECC", "#663300", false);
	//D.Draw("#FFFFFF", "#663300", false, "Click here to refresh graph", "DiagramClick()");
    ...
    for (j=1;j<73;j++)
    {
      tmpmark0=tmpmark1;
      tmpmark1=num[j];
      //new Line(D.ScreenX(j-144), D.ScreenY(tmpmark0), D.ScreenX(j-143), D.ScreenY(tmpmark1), "0000ff", 2,num[j]);
      new Box(D.ScreenX(j-47), D.ScreenY(tmpmark1), D.ScreenX(j-46), D.ScreenY(0), "0000ff",'','','','',num[j]);
    }
	j=D.ScreenX(-47)+610;
	new Bar(j, y0-10, j+220, y0+10, '', '小时', '#663300');
