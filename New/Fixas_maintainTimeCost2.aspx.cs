...
public partial class new_Fixas_maintainTimeCost2 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                DataSet ds = FixasMaintainHelper.SelectMaintainTimeCost2(txbFixasNo.Text.Trim(), txbFixasName.Text.Trim(), Request.QueryString["d1"], Request.QueryString["d2"], Convert.ToInt32(Session["plantCode"]));
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
                    AppLibrary.WriteExcel.XlsDocument doc = new AppLibrary.WriteExcel.XlsDocument();
                    doc.FileName = "report-" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
                    AppLibrary.WriteExcel.Worksheet sheet = doc.Workbook.Worksheets.Add("固定资产已完成保养时长汇总");
                    ...
                    {
                        PrintExcel(doc, sheet, rowIndex, row["ccDesc"], row["fixasType"], row["fixasSubType"], row["fixasNo"], row["fixasName"], row["maintainOrder"], row["maintainedName"], row["planDate"], row["beginDate"], row["endDate"], row["diff"], row["reportTime"]);
                        if (rowIndex == 1)
                        {
                            rowIndex += 4;
                        }
    ...
    protected void PrintExcel(AppLibrary.WriteExcel.XlsDocument doc, AppLibrary.WriteExcel.Worksheet sheet, int rowIndex, Object fixasCC, Object fixasType, Object fixasSubType,
       Object fixasNo, Object fixasName, Object maintainOrder, Object maintainedName, Object planDate, Object beginDate, Object endDate, Object diff, Object reportTime)
    {
        ...
        if (rowIndex == 1)
        {
            ...
            sheet.Cells.Add(rowIndex, 1, "已完成保养时长汇总：保养时间 " + txbMaintainDate1.Text.Trim() + " - " + txbMaintainDate2.Text.Trim(), xf);
            for (int i = 2; i <= 11; i++)
            {
            ...
            sheet.Cells.Add(rowIndex, 9, "报表开始时间", xf);
            sheet.Cells.Add(rowIndex, 10, "报表截止时间", xf);
        ...
        sheet.Cells.Add(rowIndex, 20, string.Format("{0:yyyy-MM-dd HH:mm}", endDate), xf);
    ...
    {
        DataSet ds = FixasMaintainHelper.SelectMaintainTimeCost2(txbFixasNo.Text.Trim(), txbFixasName.Text.Trim(), txbMaintainDate1.Text, txbMaintainDate2.Text.Trim(), Convert.ToInt32(Session["plantCode"]));
    ...
    {
        Response.Redirect("/new/Fixas_maintainTimeCost2.aspx?ty=export&d1=" + txbMaintainDate1.Text.Trim() + "&d2=" + txbMaintainDate2.Text.Trim() + "&rt=" + DateTime.Now.ToString());
    }
