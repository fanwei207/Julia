...
Partial Class price_pc_selectQadDoc
    Inherits System.Web.UI.Page


    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        If Not IsPostBack Then
            If (Not Nothing = (Request.QueryString("PQID"))) Then
            ...
            ElseIf (Not Nothing = (Request.QueryString("IMID"))) Then
                lbQAD.Text = Request("QADDet").ToString().Trim()
    ...
    Protected Sub btnReturn_Click(sender As Object, e As System.EventArgs) Handles btnReturn.Click
        If String.Empty.Equals(lbPQID.Text) Then
            Response.Redirect("pc_InquiryDet.aspx?IMID=" & lbIMID.Text & "&ComeFrom=" & Request("ComeFrom").ToString() & "&vender=" & Request("vender").ToString() & "&ItemCode=" & Request("ItemCode").ToString() & "&QAD=" & Request("QAD").ToString() & "&VenderName=" & Request("VenderName").ToString() & "&chkSelf=" & Request("chkSelf").ToString() & "&ddlStatus=" & Request("ddlStatus").ToString() & "&txtIMID=" & Request("txtIMID").ToString())
        End If
        If String.Empty.Equals(lbIMID.Text) Then
            Response.Redirect("pc_PriceApply.aspx?PQID=" & lbPQID.Text)
        End If
